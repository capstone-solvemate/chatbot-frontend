...
import { TestGuard } from '~/dasar/types/TestGuard'
...
export class KonektorRestApi {
  ...
  async testSend(endpoint: string, method: HttpMethod, data?: any, isRetrying: boolean = false): Promise<Response> {
    TestGuard.ensureInTestMode()

    return await this.send(endpoint, method, data, isRetrying)
  }
}
